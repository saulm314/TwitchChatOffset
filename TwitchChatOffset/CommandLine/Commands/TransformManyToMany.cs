...
public static class TransformManyToMany
{
    ...
    private static (string, ImplicitValue<long>, ImplicitValue<long>, ImplicitValue<Format>, ImplicitValue<AnchorPoint>, ImplicitValue<string>, long, bool)
        GetData(ParseResult p)
    {
        var a1 = _arguments;
        var (o1, o2, o3, o4, o5, o6, o7) = _options;
        return (p.GetValue(a1), p.GetImplicit(o1), p.GetImplicit(o2), p.GetImplicit(o3), p.GetImplicit(o4), p.GetImplicit(o5), p.GetValue(o6), p.GetValue(o7))!;
    }

    private static void Execute(ParseResult parseResult)
    {
        var (csvPath, cliStart, cliEnd, cliFormat, cliYttPosition, cliOutputDir, cliOptionPriority, quiet) = GetData(parseResult);
        ...
        foreach (TransformManyToManyCsvNullables nullableLine in CsvSerialization.Deserialize<TransformManyToManyCsvNullables>(reader))
        {
            ...
            if (line == null)
                continue;
            (string inputFile, string outputFile, long start, long end, Format format, AnchorPoint yttPosition, string outputDir) = line;
            ...
            string? output = BulkTransform.TryTransform(inputFile, input, start, end, format, yttPosition);
            if (output == null)
                continue;
